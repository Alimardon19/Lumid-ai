...
const RegisteredUser = () => {
    ...
    const handleSubmit = (e: any) => {
        ...
        service.post("/customer/saveUser", {...values, language_id: 0}).then(() => {
            api.success({message: "Thank you for registering! You can now log in and use the system.", placement: "top"});
            navigate("/login");
        }).catch((err) => {
            api.error({message: "Sorry, there was an error signing up. Please try again later.", placement: "top"});
            console.log(err);
        }).finally(() => {
